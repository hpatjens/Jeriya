...
impl CameraGroup {
    ...
    pub(crate) fn new(gpu_index_allocator: &Weak<dyn AllocateGpuIndex<Camera>>, debug_info: DebugInfo) -> Self {
        Self {
    ...
    pub fn insert_with(&mut self, camera_builder: CameraBuilder) -> camera::Result<Handle<Camera>> {
        self.camera_group
            ...
            .insert_with(|handle| {
                ...
                let result = camera_builder.build(handle.clone(), gpu_index_allocation);
                if let Err(_) = &result {
